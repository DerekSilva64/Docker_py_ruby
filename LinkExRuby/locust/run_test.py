...
def run_load_test(duration_seconds=60, users=50):
    ...
    csv_base = f"/mnt/locust/{results_dir}/results_{timestamp}"
    csv_stats = os.path.join(results_dir, f"results_{timestamp}_stats.csv")
    csv_history = os.path.join(results_dir, f"results_{timestamp}_history.csv")
    
    try:
        # Comando do Locust para executar o teste headless (sem interface web)
        cmd = [
            "docker-compose", "run", "--rm",
            "-v", f"{os.path.abspath(results_dir)}:/mnt/locust/{results_dir}",
